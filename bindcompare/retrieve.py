...
def main():
    with gzip.open("bindcompare/reference_files/dm6.fa.gz", "rb") as file_in:
        with open("dm6.fa", "wb") as file_out:
            shutil.copyfileobj(file_in, file_out)
            print("dm6 fasta file created")

    with gzip.open(
        "bindcompare/reference_files/dmel-all-r6.46.gtf.gz", "rb"
    ) as file_in:
        with open("dmel-all-r6.46.gtf", "wb") as file_out:
            shutil.copyfileobj(file_in, file_out)
            print("dm6 GTF file created")

    copy_file("bindcompare/reference_files/dm6.fa.fai")
