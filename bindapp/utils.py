        ...
        for line in table:
            ...
            if (bed_row[0])[0:2] == "Un" or (len(bed_row[0]) > 5) or (bed_row[0] == "chr"):
                ...
                if not(str.isnumeric(bed_row[1])) or not(str.isnumeric(bed_row[2])):
                    ...
                    reference_peaks[this_chrom].append((bed_row[1], bed_row[2]))
                else:
                    reference_peaks[this_chrom] = [(bed_row[1], bed_row[2])]
...
def within(exp_bind, ref_bind, scope):
    ...
    # Create the scope. 
    overlap = range(max(int(exp_bind[0]), midpoint - scope), min(int(exp_bind[1]), midpoint + scope)+1)
    if len(overlap) == 0:
        return overlap
    else:
